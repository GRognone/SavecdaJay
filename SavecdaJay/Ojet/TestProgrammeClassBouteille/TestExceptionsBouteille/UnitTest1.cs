...
namespace TestExceptionsBouteille
{
    ...
    public class UnitTest1
    {
        ...
        public void Given_quantiteAVider_When_valeurQuantiteAVider_Then_valeurQuantiteAViderAffectee()
        {
            ...
            Bouteille bouteille = new Bouteille(100, 25, true);
            //act
            bool result=bouteille.Vider(23);
            //assert
            Assert.AreEqual(2, bouteille.NiveauActuelEnCl);
            Assert.IsTrue(result);
        }

        //test pour verifier que pour tout vider la quantité a vider est = au niveau actuel.
        [TestMethod]
        public void Given_quantiteAVider_When_valeurQuantiteAVider_Then_valeurQuantiteAVIderAffecteInferieureNiveauActuel()
        {
            //arrange
            Bouteille bouteille = new Bouteille(100, 25, true);
            //act
            bool result = bouteille.Vider(25);
            //assert
            Assert.AreEqual(0, bouteille.NiveauActuelEnCl);
            Assert.IsTrue(result);
        }


        //test pour verifier que si la quantité a vider !> niveau actuel
        [TestMethod]
        public void Given_BouteilleQuantiteActuelle24_When_Vider_Quantite25_Then_ValeurDeContenuImpossibleExceptionEtQuantiteActuelleInchangee()
        {
            //arrange
            Bouteille bouteille = new Bouteille(100, 24, true);
            //act
            bool result = bouteille.Vider(25);
            //assert

            Assert.IsFalse(result);
            Assert.IsTrue(bouteille.NiveauActuelEnCl == 24, "La quantite de liquide à l'intérieur de la bouteille est inchangé");
        }
        #endregion


        #region Remplir Bouteille
        ...
        public void Given_quantiteARemplir_When_valeurQuantiteARemplir_Then_valeurQuantiteARemplirImpossibleException()
        {
            Bouteille bouteille = new Bouteille(100, 50, true);
            //assert
            Assert.ThrowsException<ArgumentOutOfRangeException>(() => bouteille.Remplir(-5));
        }

        //test pour verifier que la quantité à remplir est <= à la contenance
        [TestMethod]
        public void Given_quantitéARemplir_WhenQuantiteARemplirEstInferieurAContenance_Then_quantireARemplirValeurAffectee()
        {
            //arrange
            Bouteille bouteille = new Bouteille(100, 25, true);
            //act
            bool result =bouteille.Remplir(75);
            //assert
            Assert.AreEqual(100, bouteille.NiveauActuelEnCl);
            Assert.IsTrue(result);
        }

        // //test pour verifier que si la quantite a remplir est > à la contenance il y a une levee d'erreur. 
        [TestMethod]
        public void Given_BouteilleQuantiteActuelle25_When_quantiteARemplir75_Then_ValeurDeContenuImpossibleExceptionEtQuantiteActuelleInchangee()
        {
            //arrange
            Bouteille bouteille = new Bouteille(100, 25, true);
            //act
            bool result = bouteille.Remplir(80);
            //assert
            Assert.IsFalse(result);
            Assert.IsTrue(bouteille.NiveauActuelEnCl == 25, "La quantite de liquide à l'intérieur de la bouteille reste inchangé");
        }
        ...
        [TestMethod]
        public void Given_quantiteARemplir_When_valeurQuantiteARemplir_Then_valeurQuantiteARemplirAffecteInferieureAlAcontenanceMaxMoinsNiveauActuel()
        {
            //arrange
            Bouteille bouteille = new Bouteille(100, 25, true);
            //act
            bool result = bouteille.Remplir(25);
            //assert
            Assert.AreEqual(50, bouteille.NiveauActuelEnCl);
            Assert.IsTrue(result);
